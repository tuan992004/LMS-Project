...
export const TeacherLayout = () => {
    ...
    const navItems = [
        ...
        { path: '/teacher/announcements', label: t('nav_broadcast'), icon: Megaphone },
    ];
    ...
    return (
        <div className="flex min-h-screen font-sans text-[var(--text-primary)] transition-colors duration-500 bg-[var(--bg-primary)]">
            ...
            <aside
                ...
                `}
                aria-label={t('nav_teacher_sidebar_aria') || "Teacher Portal Sidebar"}
            >
                ...
                >
                    {navItems.map((item) => {
                        const isActive = location.pathname.startsWith(item.path) && (item.path !== '/teacher' || location.pathname === '/teacher');
                        return (
                            <Link
                                key={item.label}
                                to={item.path}
                        ...
                        {isSidebarOpen && (
                            <div className="flex-1 min-w-0 animate-fade-in">
                                ...
                                <p className="text-[9px] text-[var(--text-secondary)] uppercase tracking-wider font-black leading-relaxed truncate group-hover:text-[var(--bg-primary)]/60">
                                    {t('auth_verified') || "Mentor Verified"}
                                </p>
                            </div>
                        )}
                    </Link>
