...
export const TeacherLayout = () => {
    ...
    const navItems = [
        { path: '/teacher', label: t('nav_dashboard'), icon: LayoutDashboard },
        { path: '/teacher/courses', label: t('nav_courses'), icon: BookOpen },
        { path: '/teacher/students', label: t('nav_students'), icon: Users },
        { path: '/teacher/assignments', label: t('nav_assignments'), icon: ClipboardList },
    ];
                ...
                <div className="h-24 flex items-center justify-between px-8 border-b border-[var(--border-color)]/50 relative overflow-hidden">
                    ...
                    {isSidebarOpen ? (
                        <div className="flex items-center gap-4 animate-fade-in">
                            <div className="w-10 h-10 bg-[var(--accent-primary)] rounded-2xl flex items-center justify-center shadow-xl shadow-indigo-500/20 rotate-3 translate-y-[-2px]">
                            ...
                            <div className="flex flex-col">
                                ...
                                <span className="text-[8px] font-black uppercase tracking-[0.4em] text-[var(--text-secondary)] opacity-40">{t('portal_teacher_sub')}</span>
                            </div>
                        </div>
                    ) : (
                        <div className="w-10 h-10 bg-[var(--accent-primary)] rounded-2xl flex items-center justify-center mx-auto shadow-xl shadow-indigo-500/20 transition-transform duration-500 hover:rotate-[360deg]">
                            <Trophy className="text-white h-6 w-6" />
                        </div>
                    )}
                </div>
                    ...
                    <div className={`flex items-center gap-4 p-3 rounded-2xl bg-[var(--bg-primary)] border border-[var(--border-color)]/50 shadow-inner group ${!isSidebarOpen && 'justify-center cursor-pointer'}`}>
                        ...
                        {isSidebarOpen && (
                            <div className="flex-1 min-w-0 animate-fade-in">
                                <p className="text-sm font-black text-[var(--text-primary)] truncate italic">{user?.fullname || 'Educator'}</p>
                                <div className="flex items-center gap-2">
                                    <div className="h-1.5 w-1.5 rounded-full bg-emerald-500 blink" />
                                    <p className="text-[10px] text-[var(--text-secondary)] uppercase tracking-[0.2em] font-black opacity-40">Mentor Verified</p>
                                </div>
                            </div>
                        )}
                    </div>
            ...
            >
                ...
                <header className="h-24 glass-nav sticky top-0 z-[90] px-8 sm:px-12 flex items-center justify-between">
                    ...
                    <div className="hidden md:flex items-center gap-4 bg-[var(--bg-secondary)]/50 border border-[var(--border-color)]/50 px-6 py-3 rounded-3xl w-full max-w-lg group focus-within:bg-[var(--bg-primary)] focus-within:ring-4 focus-within:ring-[var(--accent-primary)]/10 focus-within:border-[var(--accent-primary)] transition-all">
                        ...
                        <input 
                            type="text" 
                            placeholder="Query curricula or academics..." 
                    ...
                    <aside className="absolute left-0 top-0 bottom-0 w-80 bg-[var(--nav-bg)] backdrop-blur-3xl shadow-[50px_0_100px_-20px_rgba(0,0,0,0.5)] flex flex-col animate-in slide-in-from-left duration-500 border-r border-[var(--border-color)]/30">
                        <div className="h-24 flex items-center justify-between px-8 border-b border-[var(--border-color)]/30 relative overflow-hidden">
                            ...
                            <div className="flex items-center gap-4">
                                <div className="w-10 h-10 bg-[var(--accent-primary)] rounded-xl flex items-center justify-center shadow-xl">
                                    <Trophy className="text-white h-6 w-6" />
                                </div>
                                <span className="font-black text-2xl tracking-tighter text-[var(--text-primary)] italic">{t('portal_teacher')}</span>
                            </div>
